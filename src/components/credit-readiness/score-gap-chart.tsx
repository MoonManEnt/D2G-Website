...
const PRIORITY_COLORS: Record<string, { text: string; bg: string; fill: string }> = {
  ...
  LOW: { text: "text-muted-foreground", bg: "bg-muted", fill: "bg-slate-500" },
};
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3">
          <span className="px-3 py-1 rounded-lg text-sm font-medium bg-primary/20 text-primary">
        ...
        {factors.map((factor, i) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1.5">{factor.action}</p>
            </motion.div>
          );
        })}
      </div>
