"use client";

import { cn } from "@/lib/utils";
import { Edit3, ListChecks } from "lucide-react";

interface AccountsSectionProps {
  accounts: string;
  isEditing: boolean;
  isHovered: boolean;
  editValue: string;
  onStartEdit: () => void;
  onEditChange: (value: string) => void;
  onHover: (hovered: boolean) => void;
}

export function AccountsSection({
  accounts,
  isEditing,
  isHovered,
  editValue,
  onStartEdit,
  onEditChange,
  onHover,
}: AccountsSectionProps) {
  if (isEditing) {
    return (
      <div className="relative">
        <textarea
          value={editValue || accounts}
          onChange={(e) => onEditChange(e.target.value)}
          className="w-full p-4 bg-background border border-emerald-500/50 rounded-lg text-sm font-serif leading-relaxed resize-y min-h-[200px] focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
          autoFocus
        />
      </div>
    );
  }

  // Parse accounts into a structured format if it's a list
  const lines = accounts.split("\n").filter((l) => l.trim());
  const hasListFormat = lines.some((l) => l.trim().startsWith("-") || l.trim().startsWith("*") || l.trim().match(/^\d+\./));

  return (
    <div
      className={cn(
        "relative group cursor-pointer rounded-lg transition-all duration-200 p-3 -m-3",
        isHovered && "ring-2 ring-emerald-500/20 bg-emerald-500/5"
      )}
      onMouseEnter={() => onHover(true)}
      onMouseLeave={() => onHover(false)}
      onClick={onStartEdit}
    >
      {/* Section label on hover */}
      <div className={cn(
        "absolute -top-2 left-2 px-2 py-0.5 bg-muted rounded text-[10px] font-medium text-muted-foreground uppercase tracking-wider flex items-center gap-1 transition-opacity",
        isHovered ? "opacity-100" : "opacity-0"
      )}>
        <ListChecks className="w-3 h-3" />
        Accounts
      </div>

      <div className="text-sm font-serif leading-relaxed text-foreground">
        {hasListFormat ? (
          <div className="space-y-2">
            {lines.map((line, i) => {
              const isBullet = line.trim().startsWith("-") || line.trim().startsWith("*");
              const isNumbered = line.trim().match(/^\d+\./);
              const isIndented = line.startsWith("  ") || line.startsWith("\t");

              return (
                <div
                  key={i}
                  className={cn(
                    "whitespace-pre-wrap",
                    isIndented && "pl-4 text-muted-foreground",
                    (isBullet || isNumbered) && "font-medium"
                  )}
                >
                  {line}
                </div>
              );
            })}
          </div>
        ) : (
          <div className="whitespace-pre-wrap">{accounts}</div>
        )}
      </div>
    </div>
  );
}
